...
def receber_dados():
    ...
    try:
        # Processamento ASSÍNCRONO - responde rápido e processa depois
        with dados_lock:
            # Atualiza dados em memória primeiro (rápido)
            for i in [1, 2]:
                pzem_key = f'pzem{i}'
                if pzem_key in data:
                    for k, v in data[pzem_key].items():
        ...
        print(f"⚡ Resposta enviada em {response_time:.2f}s")
