...
class Recarga(db.Model):
    ...
    def atualizar_configuracoes(self, data):
        """Atualizar configurações a partir de um dicionário"""
        # Limites e consumo
        if 'limite_pzem1' in data:
            self.limite_pzem1 = float(data['limite_pzem1'])
        if 'limite_pzem2' in data:
            self.limite_pzem2 = float(data['limite_pzem2'])
        if 'preco_kwh' in data:
            self.preco_kwh = float(data['preco_kwh'])
        if 'saldo_kwh' in data:
            self.saldo_kwh = float(data['saldo_kwh'])
        if 'prioridade_minima_emergencia' in data:
            self.prioridade_minima_emergencia = int(data['prioridade_minima_emergencia'])
        
        # Configurações financeiras
        if 'taxa_lixo' in data:
            self.taxa_lixo = float(data['taxa_lixo'])
        if 'taxa_radio' in data:
            self.taxa_radio = float(data['taxa_radio'])
        if 'iva_percent' in data:
            self.iva_percent = float(data['iva_percent'])
        
        # Telegram
        if 'telegram_bot_token' in data:
            self.telegram_bot_token = data['telegram_bot_token']
        if 'telegram_chat_id' in data:
            self.telegram_chat_id = data['telegram_chat_id']
        if 'notify_telegram' in data:
            self.notify_telegram = bool(data['notify_telegram'])
        ...
        if 'email_notificacao' in data:
            self.email_notificacao = data['email_notificacao']
        if 'notify_email' in data:
            self.notify_email = bool(data['notify_email'])
        if 'email_frequency' in data:
            self.email_frequency = data['email_frequency']
        
        # Browser
        if 'notify_browser' in data:
            self.notify_browser = bool(data['notify_browser'])
        
        # Alertas
        if 'alert_saldo_baixo' in data:
            self.alert_saldo_baixo = bool(data['alert_saldo_baixo'])
        if 'alert_consumo_pico' in data:
            self.alert_consumo_pico = bool(data['alert_consumo_pico'])
        if 'alert_reles_desligados' in data:
            self.alert_reles_desligados = bool(data['alert_reles_desligados'])
        if 'alert_pzem_offline' in data:
            self.alert_pzem_offline = bool(data['alert_pzem_offline'])
        if 'alert_erro_sistema' in data:
            self.alert_erro_sistema = bool(data['alert_erro_sistema'])
    
    def validar_configuracoes_email(self):
        """Validar se as configurações de email estão completas"""
        return all([
            self.smtp_server,
            self.smtp_port,
            self.email_sender,
            self.email_password,
            self.email_notificacao
        ])
    
    def validar_configuracoes_telegram(self):
        """Validar se as configurações do Telegram estão completas"""
        return all([
    ...
    def __repr__(self):
        return f'<Configuracao id={self.id} saldo_kwh={self.saldo_kwh}>'
