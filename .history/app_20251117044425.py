...
def gerar_relatorio():
    ...
    if periodo == "today":
        ...
        dt_start = today - timedelta(days=today.weekday())
        dt_end = today
    elif periodo == "month":
    ...
    if tipo == "consumo":
        ...
        for d, energia in rows:
            custo = energia * Configuracao.query.first().preco_kwh
            dados.append({
    ...
    # ============================
    # 4) Relatório de Picos
    # ============================
    if tipo == "picos":
        linhas = db.session.query(
            func.date(EnergyData.timestamp),
            func.max(EnergyData.power),
            func.time(EnergyData.timestamp)
        ).group_by(func.date(EnergyData.timestamp)).all()

        for d, pico, hora in linhas:
            dados.append({
                "data": d.strftime("%Y-%m-%d"),
                "pico": float(pico),
                "hora": str(hora)
    ...
    # ============================
    # 5) Relatório de Relés
    # ============================
    if tipo == "reles":
        reles = Rele.query.all()
        for r in reles:
            consumo = db.session.query(func.avg(EnergyData.power)).filter(
                EnergyData.pzem_id == r.pzem_id
            ).scalar() or 0
    ...
    if tipo == "custo":
        rows = db.session.query(
            func.date(EnergyData.timestamp),
            func.sum(EnergyData.energy).label("energia")
        ).group_by(func.date(EnergyData.timestamp)).all()
...
def api_relatorio():
    ...
    if periodo == "today":
        ...
        return jsonify({"success": False, "message": "Período inválido"})

    base = EnergyData.query.filter(
        func.date(EnergyData.timestamp) >= dt_start,
        func.date(EnergyData.timestamp) <= dt_end
    )

    if pzem != "all":
        base = base.filter(EnergyData.pzem_id == int(pzem))
    ...
    if tipo == "consumo":
        ...
        for d, energia in rows:
            dados.append({
                "data": d.strftime("%Y-%m-%d"),
                "energia": float(energia),
                "custo": float(energia * cfg.preco_kwh)
            })
    ...
    if tipo == "picos":
        rows = db.session.query(
            func.date(EnergyData.timestamp),
            func.max(EnergyData.power),
            func.min(func.time(EnergyData.timestamp))
        ).filter(
            func.date(EnergyData.timestamp) >= dt_start,
            func.date(EnergyData.timestamp) <= dt_end
        ).group_by(func.date(EnergyData.timestamp)).all()

        for d, pico, hora in rows:
            dados.append({
                "data": d.strftime("%Y-%m-%d"),
                "pico": float(pico),
                "hora": str(hora)
