...
# ✅ DETECÇÃO DO AMBIENTE RENDER (NOVO)
if os.environ.get('RENDER'):
    print("🚀 Executando em produção no Render.com")
else:
...
#  CONFIGURAÇÃO DINÂMICA DO DATABASE PARA RENDER (NOVO)
database_url = os.environ.get('DATABASE_URL')
if database_url:
    if database_url.startswith('postgres://'):
        database_url = database_url.replace('postgres://', 'postgresql://', 1)
    app.config['SQLALCHEMY_DATABASE_URI'] = database_url
    print(f"✅ Database URL configurada para Render: {database_url[:50]}...")
