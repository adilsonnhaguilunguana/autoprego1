...
def toggle_rele_state(rele_id):
    """Alterna o estado do relé (apenas funciona em modo Manual)"""
    print(f"🔌 Recebida requisição para alternar estado do relé {rele_id}")
    
    rele = Rele.query.get(rele_id)
    if not rele:
        print(f"❌ Relé {rele_id} não encontrado")
        return jsonify({"success": False, "error": "Relé não encontrado"}), 404

    try:
        print(f"📋 Relé '{rele.nome}': estado atual={rele.estado}, modo_automatico={rele.modo_automatico}")
        
        # Verifica se está em modo Manual
        if rele.modo_automatico:
            print(f"⚠️ Tentativa de controlar relé em modo automático: {rele.nome}")
            return jsonify({
                "success": False, 
                "error": f"Relé '{rele.nome}' está em modo AUTOMÁTICO. Mude para MANUAL para controlar."
            }), 400
        ...
        estado_texto = "LIGADO" if novo_estado else "DESLIGADO"
        print(f"✅ Relé '{rele.nome}' {estado_texto}. Comando enviado: {comando}")
