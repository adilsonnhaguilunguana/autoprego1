...
def testar_email(config):
    ...
    if not all([config.smtp_server, config.email_sender, config.email_password, config.email_notificacao]):
        return jsonify({"success": False, "message": "Configurações de email incompletas"})
    ...
    try:
        ...
        <hr>
        <small>Sistema de Automação Residencial</small>
        """
