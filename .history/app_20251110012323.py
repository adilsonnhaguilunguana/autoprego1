...
def atualizar_reles_payload(data):
    ...
    for r_data in data['reles']:
        ...
        # ✅ PROTEÇÃO: Se foi alterado manualmente recentemente, IGNORA
        if (rele.ultima_alteracao_manual and 
            (datetime.utcnow() - rele.ultima_alteracao_manual).total_seconds() < 15):  # 15 segundos de proteção
            print(f"🛡️  Relé '{rele.nome}' protegido (alterado manualmente há {(datetime.utcnow() - rele.ultima_alteracao_manual).total_seconds():.1f}s)")
            continue
        
        # ✅ SÓ atualiza se o estado for DIFERENTE (evita loops)
...
def debug_sincronizacao_rele(rele_id):
    ...
    protegido = tempo_desde_manual and tempo_desde_manual < 15
    ...
    return jsonify({
        ...
        "rele": {
            ...
            "protegido": protegido,
            "protegido_por_mais": max(0, 15 - tempo_desde_manual) if tempo_desde_manual else None
        }
    })
# ==========================================================
