...
def relatorio_diario():
    pzem_id = request.args.get("pzem_id", type=int, default=1)
    dia_str = request.args.get("data")  # formato YYYY-MM-DD

    if not dia_str:
        return jsonify({"error": "Data necessária (YYYY-MM-DD)"}), 400
    ...
    dados = EnergyData.query.filter(
        EnergyData.pzem_id == pzem_id,
        EnergyData.timestamp >= inicio,
        EnergyData.timestamp < fim
    ).order_by(EnergyData.timestamp.asc()).all()

    if not dados:
        return jsonify({"success": True, "dados": [], "mensagem": "Sem dados para este dia"})

    # Processamento
    potencias = [d.power for d in dados]
    energias = [d.energy for d in dados]

    potencia_media = sum(potencias) / len(potencias)
    potencia_pico = max(potencias)

    consumo_kwh = max(energias) - min(energias)
    ...
    return jsonify({
        ...
        "data": {
            ...
            "leituras": [
                {
                    "timestamps": d.timestamp.strftime("%H:%M"),
                    "power": d.power,
                    "energy": d.energy
                } for d in dados
            ]
