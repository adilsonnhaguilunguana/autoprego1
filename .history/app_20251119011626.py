...
# ==========================================================
# FUNÇÕES PARA PICOS E ENERGIA ATUAL
# ==========================================================
# ==========================================================
...
# ==========================================================
# ==========================================================
# 🔹 ROTAS PARA CONFIGURAÇÕES DE NOTIFICAÇÕES
# ==========================================================
@app.route('/config/notificacoes', methods=['POST'])
@login_required
def config_notificacoes_novo():
    """Salvar configurações completas de notificações"""
    data = request.get_json()
    if not data:
        return jsonify({"success": False, "message": "Nenhum dado recebido"}), 400
    ...
    try:
        ...
        if 'notify_browser' in data:
            config.notify_browser = bool(data.get('notify_browser', False))
        if 'email_notificacao' in data:
            config.email_notificacao = data['email_notificacao']
        if 'email_frequency' in data:
            config.email_frequency = data['email_frequency']
        
        # Configurações do Telegram
        if 'telegram_chat_id' in data:
            config.telegram_chat_id = data['telegram_chat_id']
        if 'telegram_bot_token' in data:
            # Em produção, criptografar este token
            config.telegram_bot_token = data['telegram_bot_token']
        
        # Eventos para notificar
        if 'alert_saldo_baixo' in data:
            config.alert_saldo_baixo = bool(data['alert_saldo_baixo'])
        if 'alert_consumo_pico' in data:
            config.alert_consumo_pico = bool(data['alert_consumo_pico'])
        if 'alert_reles_desligados' in data:
            config.alert_reles_desligados = bool(data['alert_reles_desligados'])
        if 'alert_pzem_offline' in data:
            config.alert_pzem_offline = bool(data['alert_pzem_offline'])
        if 'alert_erro_sistema' in data:
            config.alert_erro_sistema = bool(data['alert_erro_sistema'])

        db.session.commit()
        
        # Log da ação
        print(f"✅ Configurações de notificação salvas por {current_user.username}")
...
def get_notificacoes():
    ...
    return jsonify({
        ...
        "notify_browser": getattr(config, 'notify_browser', True),  # Campo novo
...
def testar_notificacoes():
    ...
    try:
        if tipo_teste == 'email' and config.notify_email:
            # Aqui você implementaria o envio real de email
            print(f"📧 Email de teste enviado para: {config.email_notificacao}")
            return jsonify({
                "success": True, 
                "message": f"Email de teste enviado para {config.email_notificacao}"
            })
            
        elif tipo_teste == 'telegram' and config.notify_telegram:
            # Aqui você implementaria o envio para Telegram
            print(f"📱 Mensagem de teste enviada para Telegram Chat ID: {config.telegram_chat_id}")
            return jsonify({
                "success": True, 
            ...
            # Notificação de browser é tratada no frontend
            return jsonify({
                "success": True, 
                "message": "Notificação de browser configurada"
            })
            
        else:
            return jsonify({
                "success": False, 
                "message": f"Tipo de notificação {tipo_teste} não está ativo"
            })
            
    except Exception as e:
        return jsonify({"success": False, "message": f"Erro no teste: {str(e)}"}), 500
