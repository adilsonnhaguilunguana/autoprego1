...
# ==========================================================
# 🔹 /api/reles/<id>/toggle-mode → Alternar entre Manual/Automático
# ==========================================================
@app.route('/api/reles/<int:rele_id>/toggle-mode', methods=['POST'])
@login_required
def toggle_rele_mode(rele_id):
    ...
    # ✅ VALIDAÇÃO: Verificar se pode mudar para automático
    if not rele.modo_automatico:  # Tentando mudar para automático
        if rele.limite_individual <= 0:
            return jsonify({
                "success": False, 
                "error": f"Configure um limite individual (kWh) antes de ativar o modo automático"
            }), 400

    # Alterna o modo
    rele.modo_automatico = not rele.modo_automatico
