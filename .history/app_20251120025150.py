...
# ==========================================================
        ...
        try:
            ...
            for rele in reles:
                ...
                # 🔥 CALCULAR ESTATÍSTICAS USANDOS LOGS REAIS - CORRIGIDO
                try:
                    # Converter dates para datetime para comparação
                    start_datetime = datetime.combine(dt_start, datetime.min.time())
                    end_datetime = datetime.combine(dt_end, datetime.max.time())
                    
                    logs_periodo = ReleLog.query.filter(
                        ReleLog.rele_id == rele.id,
                        ReleLog.timestamp >= start_datetime,
                        ReleLog.timestamp <= end_datetime
                    ).order_by(ReleLog.timestamp).all()

                    # Calcular tempos baseados nos logs
                    tempo_total_segundos = (end_datetime - start_datetime).total_seconds()
                    
                    tempo_ligado_segundos = 0
                    tempo_desligado_segundos = 0
                    ciclos = 0
                    ultimo_estado = rele.estado  # Estado atual como padrão
                    ultimo_timestamp = start_datetime

                    for log in logs_periodo:
                        if ultimo_estado is not None:
                            # Calcular tempo do estado anterior
                            tempo_segmento = (log.timestamp - ultimo_timestamp).total_seconds()
                            if ultimo_estado:
                                tempo_ligado_segundos += tempo_segmento
                            else:
                                tempo_desligado_segundos += tempo_segmento
                        
                        ultimo_estado = log.estado_novo
                        ultimo_timestamp = log.timestamp
                        ciclos += 1

                    # Adicionar tempo do último estado até o fim do período
                    tempo_final = (end_datetime - ultimo_timestamp).total_seconds()
                    if ultimo_estado:
                        tempo_ligado_segundos += tempo_final
                    else:
                        tempo_desligado_segundos += tempo_final

                    # Converter para horas
                    tempo_ligado_horas = tempo_ligado_segundos / 3600
                    tempo_desligado_horas = tempo_desligado_segundos / 3600
                    
                    # Formatar para exibição
                    tempo_ligado_str = f"{tempo_ligado_horas:.1f}h"
                    tempo_desligado_str = f"{tempo_desligado_horas:.1f}h"
                    eficiencia = (tempo_ligado_horas / (tempo_total_segundos / 3600)) * 100 if tempo_total_segundos > 0 else 0

                except Exception as log_error:
                    print(f"⚠️ Erro ao calcular logs do relé {rele.id}: {log_error}")
                    # Fallback para dados básicos
                    tempo_ligado_str = "N/A"
                    tempo_desligado_str = "N/A"
                    ciclos = 0
                    eficiencia = 0
                ...
                dados.append({
                    ...
                    "tempo_ligado": tempo_ligado_str,
                    "tempo_desligado": tempo_desligado_str,
                    "ciclos": ciclos,
                    "eficiencia_tempo": f"{eficiencia:.1f}%",
                    "total_mudancas": len(logs_periodo) if 'logs_periodo' in locals() else 0,
                    "periodo_analisado": f"{(tempo_total_segundos / 3600 / 24):.1f} dias" if 'tempo_total_segundos' in locals() else "N/A"
                })
            ...
            return jsonify({
                ...
                "metadata": {
                    ...
                    "dias_analisados": (tempo_total_segundos / 3600 / 24) if 'tempo_total_segundos' in locals() and tempo_total_segundos > 0 else 0
                }
            })
        except Exception as e:
            print(f"❌ Erro completo no relatório de relés: {traceback.format_exc()}")
