...
# ==========================================================
# RELATORIOS - ROTA ÚNICA E CONSOLIDADA
# ==========================================================
...
def gerar_relatorio():
    ...
    # ============================
    # 5) Relatório de Desempenho por Relé
    # ============================
    elif tipo == "reles":
        try:
            ...
            for rele in reles:
                # Calcular consumo médio do PZEM associado
                consumo_medio = db.session.query(func.avg(EnergyData.power)).filter(
                    EnergyData.pzem_id == rele.pzem_id,
                    func.date(EnergyData.timestamp) >= dt_start,
                    func.date(EnergyData.timestamp) <= dt_end
                ).scalar() or 0

                # Calcular tempo de operação (estimado)
                tempo_ligado = "N/A"  # Poderia ser implementado com logs de estado
...
def save_notificacoes_config():
    ...
    try:
        ...
        config.notify_telegram = data.get('notify_telegram', config.notify_telegram)
        ...
        config.notify_email = data.get('notify_email', config.notify_email)
        config.smtp_server = data.get('smtp_server', config.smtp_server)
        config.smtp_port = data.get('smtp_port', config.smtp_port)
        config.email_sender = data.get('email_sender', config.email_sender)
        config.email_password = data.get('email_password', config.email_password)
        config.email_notificacao = data.get('email_notificacao', config.email_notificacao)
        config.email_frequency = data.get('email_frequency', config.email_frequency)
        
        # Configurações do Browser
        config.notify_browser = data.get('notify_browser', config.notify_browser)
        
        # Alertas específicos
        config.alert_saldo_baixo = data.get('alert_saldo_baixo', config.alert_saldo_baixo)
        config.alert_consumo_pico = data.get('alert_consumo_pico', config.alert_consumo_pico)
        config.alert_reles_desligados = data.get('alert_reles_desligados', config.alert_reles_desligados)
        config.alert_pzem_offline = data.get('alert_pzem_offline', config.alert_pzem_offline)
        config.alert_erro_sistema = data.get('alert_erro_sistema', config.alert_erro_sistema)
...
def testar_email(config):
    ...
    campos_vazios = []
    if not config.smtp_server or config.smtp_server.strip() == "":
        campos_vazios.append("Servidor SMTP")
    if not config.smtp_port:
        campos_vazios.append("Porta SMTP")
    if not config.email_sender or config.email_sender.strip() == "":
        campos_vazios.append("Email de Envio")
    if not config.email_password or config.email_password.strip() == "":
        campos_vazios.append("Senha do Email")
    if not config.email_notificacao or config.email_notificacao.strip() == "":
        campos_vazios.append("Email para Notificações")
    
    if campos_vazios:
        mensagem_erro = f"Campos obrigatórios não preenchidos: {', '.join(campos_vazios)}"
    ...
    try:
        ...
        print(f"🔄 Conectando ao servidor SMTP {config.smtp_server}:{config.smtp_port}...")
        ...
        print(f"🔐 Efetuando login com {config.email_sender}...")
        ...
        erro = "Falha na autenticação. Verifique o email e senha."
        print(f"❌ Erro de autenticação: {e}")
        return jsonify({"success": False, "message": f"{erro} Detalhes: {str(e)}"})
    
    except smtplib.SMTPConnectError as e:
        erro = "Não foi possível conectar ao servidor SMTP."
        print(f"❌ Erro de conexão: {e}")
        return jsonify({"success": False, "message": f"{erro} Verifique o servidor e porta."})
    
    except Exception as e:
        print(f"❌ Erro ao enviar email: {e}")
        return jsonify({"success": False, "message": f"Erro ao enviar email: {str(e)}"}) 

@app.route('/config/notificacoes/debug', methods=['GET'])
@login_required
def debug_notificacoes():
    """Debug das configurações de notificação"""
    config = Configuracao.query.first()
    if not config:
    ...
    return jsonify({
        "email_config": {
            ...
            "email_password_length": len(config.email_password) if config.email_password else 0,
            "email_notificacao": config.email_notificacao,
            "notify_email": config.notify_email,
            "email_frequency": config.email_frequency
        },
        "telegram_config": {
            "telegram_bot_token_length": len(config.telegram_bot_token) if config.telegram_bot_token else 0,
