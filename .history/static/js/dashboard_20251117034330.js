...
function gerarRelatorio() {
    ...
    const data = document.getElementById("report-date").value;

    if (!data && (tipo === "diario" || tipo === "semanal" || tipo === "mensal")) {
        mostrarToast("Selecione uma data!", "danger");
        return;
    }

    fetch(`/relatorios/gerar?tipo=${tipo}&data=${data}`)
        .then(response => response.json())
        .then(result => {
            const cont = document.getElementById("report-result");
        ...
        .catch(err => mostrarToast("Erro ao gerar relatório!", "danger"));
}
function exportarPDF() {
    window.open("/relatorios/export/pdf", "_blank");
}

function exportarExcel() {
    window.open("/relatorios/export/excel", "_blank");
}
