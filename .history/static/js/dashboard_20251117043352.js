...
async function gerarRelatorio() {
    ...
    if (periodo === "custom") {
        if (!startDate || !endDate) {
            showToast("Selecione intervalo de datas!", "warning");
            return;
        }
    }
    ...
    try {
        ...
        renderizarTabelaRelatorio(data.dados, tipo);
...
function renderizarTabelaRelatorio(linhas, tipo) {
    const cont = document.getElementById("report-results");
    cont.innerHTML = "";

    let html = `
        <table class="table table-hover reles-table">
            <thead>
                <tr>
                    ${tipo === "consumo" ? "<th>Data</th><th>Energia (kWh)</th><th>Custo</th>" :
                      tipo === "picos" ? "<th>Data</th><th>Pico (W)</th><th>Hora</th>" :
                      "<th>Data</th><th>Valor (MZN)</th><th>kWh</th>"}
                </tr>
            </thead>
            <tbody>
    `;
...
function exportarPDF() {
    window.open("/api/relatorio/export/pdf");
}

// Exportar Excel
function exportarExcel() {
    window.open("/api/relatorio/export/excel");
}
