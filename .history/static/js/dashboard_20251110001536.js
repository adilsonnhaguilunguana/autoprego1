...
async function alternarEstadoRele(releId, estadoAtual) {
    ...
    // Verificar modo atual do relé
    const releMode = releControlState.modos[releId];
    
    if (!releMode) {
        console.warn(`Modo do relé ${releId} não encontrado, recarregando modos...`);
        await carregarModosReles();
        return alternarEstadoRele(releId, estadoAtual);
    }

    // Verificar se está em modo Manual
    if (releMode.modo_automatico) {
    ...
    try {
        ...
        if (data.success) {
            ...
            // ✅ ATUALIZAÇÃO IMEDIATA na interface (otimista)
        ...
        // Recarregar para sincronizar estado real
        await carregarReles();
    }
}
