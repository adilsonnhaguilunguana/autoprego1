...
function renderRelatorioReles(dados, metadata) {
    ...
    dados.forEach(rele => {
        const estadoClass = rele.estado === 'LIGADO' ? 'success' : 'secondary';
        const modoClass = rele.modo === 'Automático' ? 'info' : 'warning';
        
        html += `
            <tr>
                <td><strong>${rele.nome}</strong></td>
                <td>PZEM ${rele.pzem_id}</td>
                <td><span class="badge bg-${estadoClass}">${rele.estado}</span></td>
                <td><span class="badge bg-${modoClass}">${rele.modo}</span></td>
                <td>${rele.consumo_medio} W</td>
                <td><span class="text-success"><i class="bi bi-clock"></i> ${rele.tempo_ligado || 'N/A'}</span></td>
                <td><span class="text-secondary"><i class="bi bi-clock"></i> ${rele.tempo_desligado || 'N/A'}</span></td>
                <td>${rele.ciclos || rele.mudancas_estado || 0}</td>
                <td>
                    <div class="d-flex align-items-center">
                        <div class="progress flex-grow-1" style="height: 15px;">
                            <div class="progress-bar bg-success" role="progressbar" 
                                 style="width: ${rele.eficiencia_tempo ? parseFloat(rele.eficiencia_tempo) : 0}%" 
                                 aria-valuenow="${rele.eficiencia_tempo ? parseFloat(rele.eficiencia_tempo) : 0}" 
                                 aria-valuemin="0" aria-valuemax="100">
                            </div>
                        </div>
                        <small class="ms-2">${rele.eficiencia_tempo || '0%'}</small>
                    </div>
                </td>
            </tr>
        `;
