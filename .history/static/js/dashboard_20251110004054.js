...
async function alternarEstadoRele(releId, estadoAtual) {
    console.log(`🔌 Tentando alternar relé ${releId} de ${estadoAtual ? 'LIGADO' : 'DESLIGADO'} para ${!estadoAtual ? 'LIGADO' : 'DESLIGADO'}`);
    
    // ✅ PRIMEIRO: Garantir que está em modo Manual
    try {
        const modoResponse = await fetch(`/api/reles/${releId}/modo`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ modo_automatico: false })
        });
        
        if (!modoResponse.ok) {
            throw new Error('Não foi possível mudar para modo manual');
        }
        
        const modoData = await modoResponse.json();
        if (!modoData.success) {
            throw new Error(modoData.error || 'Erro ao mudar modo');
        }
        
        console.log('✅ Modo alterado para MANUAL');
        
    } catch (error) {
        console.error('❌ Erro ao mudar modo:', error);
        showToast('Erro ao configurar modo manual', 'warning');
        return;
    }

    // AGORA: Alternar o estado
    try {
        // Mostrar loading no botão
        const button = document.querySelector(`button[onclick*="alternarEstadoRele(${releId}, ${estadoAtual})"]`);
        ...
        console.log(`📤 Enviando comando para relé ${releId}...`);
        ...
        if (data.success) {
            ...
            const releRow = document.querySelector(`tr:has(button[onclick*="alternarEstadoRele(${releId}, ${estadoAtual})"])`);
            if (releRow) {
                // Atualizar estado
                const estadoBadge = releRow.querySelector('.badge');
                if (estadoBadge) {
                    const novoEstado = !estadoAtual;
                    estadoBadge.className = novoEstado ? 'badge bg-success' : 'badge bg-secondary';
                    estadoBadge.textContent = novoEstado ? 'Ligado' : 'Desligado';
                }
                
                // Atualizar botão de modo para mostrar que está manual
                const modoBadge = releRow.querySelector('td:nth-child(4) .badge');
                if (modoBadge) {
                ...
                const powerButton = releRow.querySelector(`button[onclick*="alternarEstadoRele(${releId}, ${estadoAtual})"]`);
                if (powerButton) {
                    powerButton.className = `btn btn-action ${!estadoAtual ? 'btn-outline-danger' : 'btn-outline-success'}`;
                    powerButton.setAttribute('onclick', `alternarEstadoRele(${releId}, ${!estadoAtual})`);
                    powerButton.innerHTML = '<i class="bi bi-power"></i>';
                    powerButton.title = !estadoAtual ? 'Desligar' : 'Ligar';
                    
                    // Atualizar tooltip
                    const tooltipInstance = bootstrap.Tooltip.getInstance(powerButton);
                    if (tooltipInstance) {
                        tooltipInstance.dispose();
                    }
                    new bootstrap.Tooltip(powerButton);
                }
            }
    ...
    // ✅ Recarrega dados completos após 2 segundos
    setTimeout(async () => {
        await carregarReles();
        await carregarModosReles();
    }, 2000);
...
async function carregarReles() {
        const tbody = document.getElementById('reles-table-body');
    if (tbody) {
        tbody.innerHTML = '';
    }
    ...
    try {
        ...
        if (data.success && Array.isArray(data.reles)) {
         // Garante que a lista é limpa antes de atualizar
            state.reles.allReles = [];
            state.reles.filteredReles = [];
