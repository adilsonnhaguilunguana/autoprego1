    ...
    try:
        with dados_lock:  # Protege contra acesso simultâneo
           ...
           # atualizar_saldo_com_consumo()
            ...
            #energy_data_entries = []  # Lista para armazenar os dados a serem salvos
            ...
            for i in [1, 2]:
                ...
                if pzem_key in data:
                    ...
                    # Prepara dados para salvar no banco (apenas se estiver conectado)
                    if dados_pzem[pzem_key]['conectado']:
                        try:
            ...
            # PASSO 3: Salvar dados no banco (uma única transação)
            if energy_data_entries:
                try:
                    for energy_data in energy_data_entries:
                        db.session.add(energy_data)
                    db.session.commit()
                    print(f"✅ {len(energy_data_entries)} registros salvos no banco de dados")
                except Exception as e:
                    print(f"❌ Erro ao salvar EnergyData no banco: {e}")
                    print(traceback.format_exc())
                    db.session.rollback()

            # PASSO 4: Atualizar estados dos relés (se enviados pelo ESP)
            if 'reles' in data:
            ...
            # PASSO 6: Salvar alterações dos relés
            try:
                db.session.commit()
            ...
            return jsonify({"status": "success", "message": "Dados recebidos e processados"})
