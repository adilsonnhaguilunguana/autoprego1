...
def verificar_e_controlar_reles():
    """Liga ou desliga relés AUTOMÁTICOS baseado no saldo kWh.
       Esta versão é SEGURA e sempre envia OFF quando crítico.
    """
    ...
    print(f"\n🔋 SALDO GLOBAL: {saldo:.2f} kWh")
    reles = Rele.query.all()

    PROTECAO_SEGUNDOS = 5   # antes era 30 — agora responde mais rápido

    for rele in reles:

        print(f"\n➡️ Verificando relé '{rele.nome}' (ID {rele.id})")

        # 1️⃣ Se estiver em MODO MANUAL → ignorar completamente
        if not rele.modo_automatico:
            print("⛔ Modo Manual — ignorando.")
            continue

        # 2️⃣ Proteção após ação manual
        if rele.ultima_alteracao_manual:
            delta = (datetime.utcnow() - rele.ultima_alteracao_manual).total_seconds()
            if delta < PROTECAO_SEGUNDOS:
                print(f"🛡️ Proteção ativa ({delta:.1f}s) — ignorando alterações.")
                continue

        # 3️⃣ Caso CRÍTICO → SEMPRE DESLIGAR
        if saldo <= rele.limite_individual:

            print(f"⚠️ LIMITE ATINGIDO! SALDO({saldo:.2f}) ≤ LIMITE({rele.limite_individual})")

            # se banco diz ligado → muda estado
            if rele.estado:
                print(f"⚡ Desligando '{rele.nome}' (mudança real).")
                rele.estado = False
            else:
                print(f"⚡ '{rele.nome}' já está desligado no banco — reenviando OFF para garantir.")

            # SEMPRE enviar comando OFF
            with dados_lock:
                comandos_pendentes.append(f"RELE{rele.id}_OFF")
                print(f"📤 Comando enviado → RELE{rele.id}_OFF")

            continue

        # 4️⃣ Caso saldo esteja normal → ligar se estiver desligado
        if saldo > rele.limite_individual:

            print(f"🟢 SALDO OK! SALDO({saldo:.2f}) > LIMITE({rele.limite_individual})")

            if not rele.estado:
                print(f"🔛 Ligando '{rele.nome}' (mudança real).")
                rele.estado = True

                with dados_lock:
                    comandos_pendentes.append(f"RELE{rele.id}_ON")
                    print(f"📤 Comando enviado → RELE{rele.id}_ON")

            else:
                print(f"🔛 '{rele.nome}' já está ligado — nada a fazer.")
    ...
    try:
        db.session.commit()
        print("💾 Estados dos relés salvos com sucesso.\n")
    except Exception as e:
