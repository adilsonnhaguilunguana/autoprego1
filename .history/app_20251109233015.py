...
# ==========================================================
# 🔹 /api/reles/<id>/toggle-state → Alternar estado (apenas Manual) - NOVA ROTA
# ==========================================================
...
def receber_dados():
    ...
    try:
        # PROCESSAMENTO RÁPIDO - sem lock inicial
        print("🔄 Processando payload dos PZEMs...")
        
        # ✅✅✅ CHAMADA SEGURA - fora do lock principal
        ...
        # PASSO 2: Preparar dados para banco (RÁPIDO)
        energy_entries = []
        for i in [1, 2]:
            pzem_key = f'pzem{i}'
        ...
        # PASSO 3: Operações de banco em bloco (ÚNICO COMMIT)
        if energy_entries:
            try:
        ...
        # PASSO 4: Atualizar relés (RÁPIDO)
        if 'reles' in data:
            print(f"🔔 Atualizando estados dos relés: {data['reles']}")
            for r in data['reles']:
                rele = Rele.query.get(r['id'])
        ...
        # PASSO 5: Commitar mudanças dos relés
        try:
            db.session.commit()
            print("✅ Estados dos relés atualizados")
        except Exception as e:
            print(f"❌ Erro ao salvar estados dos relés: {e}")
            db.session.rollback()
