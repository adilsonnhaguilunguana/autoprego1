...
def atualizar_reles_payload(data):
    ...
    for r_data in data['reles']:
        ...
        # ⏱️ JANELA DE PROTEÇÃO (30s após comando manual)
        PROTECAO_SEGUNDOS = 30
        
        # ✅ PROTEÇÃO: se foi alterado manualmente há pouco, IGNORA COMPLETAMENTE
        if (rele.ultima_alteracao_manual and 
            (datetime.utcnow() - rele.ultima_alteracao_manual).total_seconds() < PROTECAO_SEGUNDOS):
            print(
                f"🛡️  Relé '{rele.nome}' PROTEGIDO - "
                f"Alterado manualmente há "
                f"{(datetime.utcnow() - rele.ultima_alteracao_manual).total_seconds():.1f}s - "
                f"IGNORANDO atualização do ESP"
            )
            continue
    ...
    try:
        db.session.commit()
        print("✅ Estados dos relés atualizados no banco (após proteção)")
    except Exception as e:
