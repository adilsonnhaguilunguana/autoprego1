...
# ==========================================================
        ...
        try:
            ...
            for rele in reles:
                ...
                # 🔥 CALCULAR ESTATÍSTICAS USANDOS LOGS REAIS
                logs_periodo = ReleLog.query.filter(
                    ReleLog.rele_id == rele.id,
                    ReleLog.timestamp >= datetime.combine(dt_start, datetime.min.time()),
                    ReleLog.timestamp <= datetime.combine(dt_end, datetime.max.time())
                ).order_by(ReleLog.timestamp).all()

                # Calcular tempos baseados nos logs
                tempo_total_segundos = (datetime.combine(dt_end, datetime.max.time()) - 
                                    datetime.combine(dt_start, datetime.min.time())).total_seconds()
                
                tempo_ligado_segundos = 0
                tempo_desligado_segundos = 0
                ciclos = 0
                ultimo_estado = None
                ultimo_timestamp = datetime.combine(dt_start, datetime.min.time())
                ...
                if ultimo_estado is not None:
                    tempo_final = (datetime.combine(dt_end, datetime.max.time()) - ultimo_timestamp).total_seconds()
                    if ultimo_estado:
                        tempo_ligado_segundos += tempo_final
                    else:
                        tempo_desligado_segundos += tempo_final
                else:
                    # Se não há logs, usar estado atual
                    if rele.estado:
                        tempo_ligado_segundos = tempo_total_segundos
                    else:
                        tempo_desligado_segundos = tempo_total_segundos
                ...
                dados.append({
                    ...
                    "total_mudancas": len(logs_periodo),
                    "periodo_analisado": f"{(tempo_total_segundos / 3600 / 24):.1f} dias"
                })
            ...
            return jsonify({
                ...
                "metadata": {
                    ...
                    "dias_analisados": (tempo_total_segundos / 3600 / 24) if tempo_total_segundos > 0 else 0
                }
            })
        except Exception as e:
