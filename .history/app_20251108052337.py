...
# ---------------------------------------------------------
# Helpers: validação e preparação dos dados a salvar
# ---------------------------------------------------------
def validar_payload(data):
    """Valida JSON e api_key. Retorna (ok: bool, response: flask.Response|None)."""
    if not data:
...
def preparar_energy_entries(data):
    """
    Percorre pzem1/pzem2 em payload, atualiza dados_pzem e cria instâncias EnergyData.
    Retorna lista de EnergyData (pode ser vazia).
    """
    energy_data_entries = []
    for i in [1, 2]:
        pzem_key = f'pzem{i}'
        if pzem_key in data:
            ...
            # Atualiza campos locais em dados_pzem
...
def salvar_energy_entries(entries):
    """Salva lista de EnergyData em uma única transação; retorna True se ok."""
    if not entries:
        return True
    try:
        for e in entries:
...
def atualizar_reles_payload(data):
    """Atualiza objetos Rele com estados vindos no payload (não comita aqui)."""
...
# ---------------------------------------------------------
# Processor: encapsula a lógica complexa (usa dados_lock)
# ---------------------------------------------------------
def processar_payload(data):
    """
    Processa todo o payload: atualizar saldo, preparar/salvar energy entries,
    atualizar relés, verificar e controlar, e salvar estados dos relés.
    Tudo protegido por dados_lock para evitar concorrência.
    """
    try:
        with dados_lock:
            ...
            # PASSO 4: Atualizar estados dos relés (vindo do ESP)
...
# ---------------------------------------------------------
# Rota refatorada: curta, com guard clauses
# ---------------------------------------------------------
@app.route('/api/dados', methods=['POST'])
def receber_dados():
    """
    Recebe o JSON do ESP, valida e delega processamento para processar_payload.
    Função curta e legível; heavy-lifting está em helpers.
    """
    try:
    ...
    ok, resp = validar_payload(data)
    if not ok:
        return resp  # já é um (Response, status)

    # Processa (síncrono). Se preferires, podemos rodar processar_payload em thread.
    success = processar_payload(data)
    if success:
