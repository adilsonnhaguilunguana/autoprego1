...
# ==========================================================
# ==========================================================
# CONTROLE DE RELES - VERSÃO CORRIGIDA
# ==========================================================
...
def toggle_rele_state(rele_id):
    """Alterna o estado do relé (apenas funciona em modo Manual) - VERSÃO CORRIGIDA"""
    rele = Rele.query.get(rele_id)
    if not rele:
        return jsonify({"success": False, "error": "Relé não encontrado"}), 404
    ...
    try:
        ...
        # ✅✅✅ MARCA COMO ALTERADO MANUALMENTE (PROTEÇÃO CONTRA SINCRONIZAÇÃO)
        rele.ultima_alteracao_manual = datetime.utcnow()
        rele.bloqueado_para_sincronizacao = True
        ...
        print(f"✅ Comando MANUAL enviado para ESP: {comando} | Estado: {estado_texto}")
        
        # ✅ Agenda desbloqueio após 15 segundos (COM IMPORT CORRETO)
        def desbloquear_sincronizacao(rele_id):
            time.sleep(15)  # 15 segundos de proteção
            with app.app_context():
                rele = Rele.query.get(rele_id)
                if rele:
                    rele.bloqueado_para_sincronizacao = False
                    db.session.commit()
                    print(f"🔓 Relé {rele_id} desbloqueado para sincronização")
        
        import threading
        threading.Thread(target=desbloquear_sincronizacao, args=(rele_id,)).start()
        ...
        db.session.rollback()
        print(f"❌ Erro ao alternar estado do relé {rele_id}: {e}")
...
def atualizar_reles_payload(data):
    """Atualiza estados dos relés APENAS se vierem do ESP (com proteção para controles manuais)"""
    if 'reles' not in data:
        return
    
    print(f"🔔 Estados dos relés recebidos do ESP: {data['reles']}")
    ...
    for r_data in data['reles']:
        ...
        estado_atual = rele.estado
        
        # ✅ PROTEÇÃO 1: Se está bloqueado para sincronização, IGNORA
        if rele.bloqueado_para_sincronizacao:
            print(f"🛡️  Relé '{rele.nome}' BLOQUEADO para sincronização (controle manual recente)")
            continue
        
        # ✅ PROTEÇÃO 2: Se foi alterado manualmente recentemente, IGNORA
        if (rele.ultima_alteracao_manual and 
            (datetime.utcnow() - rele.ultima_alteracao_manual).total_seconds() < 10):
            print(f"🛡️  Relé '{rele.nome}' protegido (alterado manualmente há menos de 10s)")
            continue
        
        # ✅ PROTEÇÃO 3: Se o estado já está igual, IGNORA
        if estado_esp == estado_atual:
            print(f"✅ Estado do relé '{rele.nome}' já está sincronizado: {'LIGADO' if estado_atual else 'DESLIGADO'}")
            continue
        
        # ✅ ATUALIZA: Apenas se passou por todas as proteções
        print(f"🔄 ESP: Relé '{rele.nome}' mudou de {estado_atual} para {estado_esp}")
