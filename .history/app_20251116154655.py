...
def verificar_e_controlar_reles():
    ...
    try:
        ...
        # ✅ SOMENTE RELÉS EM MODO AUTOMÁTICO
        ...
        for rele in reles_automaticos:
            # ✅ VERIFICA: relé está LIGADO E saldo global ≤ limite individual EM kWh
            if (rele.estado and 
                ...
                print(
                    f"🤖 [AUTO] Desligando '{rele.nome}' - "
                    f"Saldo Global ({saldo_global:.2f}kWh) ≤ Limite ({rele.limite_individual}kWh)"
                )
...
def toggle_rele_state(rele_id):
    """Alterna o estado do relé (apenas funciona em modo Manual) - VERSÃO PROTEGIDA"""
    rele = Rele.query.get(rele_id)
    if not rele:
        return jsonify({"success": False, "error": "Relé não encontrado"}), 404
    ...
    try:
        ...
        # ✅ MARCA COMO ALTERADO MANUALMENTE (PROTEÇÃO POR TIMESTAMP)
        ...
        return jsonify({
            ...
            "rele": rele.to_dict(),
            # ⏱️ Proteção de 30 segundos (vamos alinhar com a função abaixo)
            "protegido_ate": (datetime.utcnow() + timedelta(seconds=30)).isoformat()
        })
    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "error": str(e)}), 500
