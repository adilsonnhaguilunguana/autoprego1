...
def testar_email(config):
    ...
    if not config.smtp_server or not config.smtp_server.strip():
        return jsonify({"success": False, "message": "Servidor SMTP não configurado"})
    
    if not config.smtp_port or config.smtp_port <= 0:
        return jsonify({"success": False, "message": "Porta SMTP inválida"})
    ...
    try:
        ...
        <ul>
            ...
            <li>Tipo: {'SSL' if config.smtp_port == 465 else 'TLS'}</li>
        </ul>
        ...
        if config.smtp_port == 465:
            # SSL - conexão segura desde o início
            print("🔒 Usando SSL (porta 465)")
            server = smtplib.SMTP_SSL(config.smtp_server, config.smtp_port, timeout=15)
        else:
            # TLS - conexão normal + upgrade para segurança
            print("🔐 Usando TLS (porta 587 ou outras)")
        ...
        error_msg = f"Não foi possível conectar ao servidor {config.smtp_server}:{config.smtp_port}"
        print(f"❌ {error_msg}: {e}")
        return jsonify({"success": False, "message": error_msg})
