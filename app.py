...
def salvar_picos_diarios():
    """Salva picos DIÁRIOS automaticamente quando novos recordes são detectados"""
    try:
        hoje = datetime.utcnow().date()
        print(f"📊 Verificando picos diários para {hoje}...")
        
        for pzem_id in [1, 2]:
            pzem_key = f'pzem{pzem_id}'
            potencia_atual = dados_pzem[pzem_key]['power']
            
            if potencia_atual <= 0:
                continue
                
            # Buscar maior potência do dia atual no banco
            pico_hoje_db = EnergyData.query.filter(
                EnergyData.pzem_id == pzem_id,
                db.func.date(EnergyData.timestamp) == hoje
            ).order_by(EnergyData.power.desc()).first()
            
            # Usar o maior entre o atual e o do banco
            if pico_hoje_db and pico_hoje_db.power > potencia_atual:
                potencia_maxima = pico_hoje_db.power
                timestamp_pico = pico_hoje_db.timestamp
            else:
                potencia_maxima = potencia_atual
                timestamp_pico = datetime.utcnow()
            
            # Verificar se já existe um pico registrado para hoje neste PZEM
            pico_existente = DailyPeak.query.filter(
                DailyPeak.date == hoje,
                DailyPeak.pzem_id == pzem_id
            ).first()
            
            # Se não existe pico OU se encontrou um novo pico maior
            if not pico_existente or potencia_maxima > pico_existente.value:
                
                if pico_existente:
                    # Atualizar pico existente
                    pico_existente.value = potencia_maxima
                    pico_existente.time = timestamp_pico.time()
                    print(f"🔄 Pico DIÁRIO PZEM{pzem_id} ATUALIZADO: {pico_existente.value:.1f}W → {potencia_maxima:.1f}W")
                else:
                    # Criar novo pico diário
                    novo_pico = DailyPeak(
                        date=hoje,
                        pzem_id=pzem_id,
                        value=potencia_maxima,
                        time=timestamp_pico.time()
                    )
                    db.session.add(novo_pico)
                    print(f"✅ NOVO pico DIÁRIO PZEM{pzem_id}: {potencia_maxima:.1f}W às {timestamp_pico.time().strftime('%H:%M')}")
            
            else:
                # Apenas para debug - mostrar que verificou mas não atualizou
                print(f"📈 PZEM{pzem_id}: {potencia_atual:.1f}W (Pico atual: {pico_existente.value:.1f}W)")
        
        db.session.commit()
        print("💾 Picos diários verificados e salvos!")
        
    except Exception as e:
        print(f"❌ Erro ao salvar picos diários: {e}")
        db.session.rollback()
...
def salvar_picos_semanais():
    """Salva picos semanais automaticamente"""
    try:
        hoje = datetime.utcnow().date()
        inicio_semana = hoje - timedelta(days=hoje.weekday())  # Segunda-feira
        
        for pzem_id in [1, 2]:
            # Buscar maior pico da semana
            pico_semana = EnergyData.query.filter(
                EnergyData.pzem_id == pzem_id,
            ...
            # Verificar se já existe pico semanal
            pico_existente = WeeklyPeak.query.filter(
                WeeklyPeak.week_start == inicio_semana,
                WeeklyPeak.pzem_id == pzem_id
            ).first()
            
            # Se não existe OU encontrou pico maior
            if not pico_existente or pico_semana.power > pico_existente.value:
                
                if pico_existente:
                    # Atualizar pico existente
                    pico_existente.value = pico_semana.power
                    pico_existente.time = pico_semana.timestamp.time()
                    print(f"🔄 Pico semanal PZEM{pzem_id} atualizado: {pico_semana.power}W")
                else:
                    # Criar novo pico semanal
                    novo_pico = WeeklyPeak(
                        week_start=inicio_semana,
                        pzem_id=pzem_id,
                        value=pico_semana.power,
                        time=pico_semana.timestamp.time()
                    )
                    db.session.add(novo_pico)
                    print(f"✅ Novo pico semanal PZEM{pzem_id}: {pico_semana.power}W")
        
        db.session.commit()
        
    except Exception as e:
        print(f"❌ Erro ao salvar picos semanais: {e}")
        db.session.rollback()
...
def salvar_picos_mensais():
    """Salva picos mensais automaticamente"""
    try:
        hoje = datetime.utcnow().date()
        inicio_mes = hoje.replace(day=1)
        
        for pzem_id in [1, 2]:
            # Buscar maior pico do mês
            pico_mes = EnergyData.query.filter(
                EnergyData.pzem_id == pzem_id,
                EnergyData.timestamp >= inicio_mes
            ).order_by(EnergyData.power.desc()).first()
            ...
            # Verificar se já existe pico mensal
            pico_existente = MonthlyPeak.query.filter(
                MonthlyPeak.month == hoje.month,
                MonthlyPeak.year == hoje.year,
                MonthlyPeak.pzem_id == pzem_id
            ).first()
            
            # Se não existe OU encontrou pico maior
            if not pico_existente or pico_mes.power > pico_existente.value:
                
                if pico_existente:
                    # Atualizar pico existente
                    pico_existente.value = pico_mes.power
                    pico_existente.time = pico_mes.timestamp.time()
                    print(f"🔄 Pico mensal PZEM{pzem_id} atualizado: {pico_mes.power}W")
                else:
                    # Criar novo pico mensal
                    novo_pico = MonthlyPeak(
                        month=hoje.month,
                        year=hoje.year,
                        pzem_id=pzem_id,
                        value=pico_mes.power,
                        time=pico_mes.timestamp.time()
                    )
                    db.session.add(novo_pico)
                    print(f"✅ Novo pico mensal PZEM{pzem_id}: {pico_mes.power}W")
        
        db.session.commit()
        
    except Exception as e:
        print(f"❌ Erro ao salvar picos mensais: {e}")
        db.session.rollback()
...
def receber_dados():
    ...
    try:
        ...
        # ✅✅✅ PASSO 4: SALVAR PICOS DIÁRIOS, SEMANAIS E MENSAIS (CRÍTICO)
        salvar_picos_automaticamente()

        # PASSO 5: Atualizar estados dos relés (SE HOUVER DADOS)
        if 'reles' in data:
            for r in data['reles']:
        ...
        # PASSO 6: ✅ VERIFICAÇÃO DE NOTIFICAÇÕES (fora do tempo crítico)
...
# ==========================================================
# SISTEMA DE ARMAZENAMENTO DE PICOS
# ==========================================================
def verificar_e_salvar_picos():
    """Verifica e salva TODOS os tipos de picos automaticamente"""
    print("🏔️ INICIANDO VERIFICAÇÃO DE PICOS...")
    salvar_picos_diarios()      # ✅ AGORA INCLUI DIÁRIOS!
    salvar_picos_semanais() 
    salvar_picos_mensais()
    print("🏔️ VERIFICAÇÃO DE PICOS CONCLUÍDA!")
def salvar_picos_automaticamente():
    """Salva picos diários, semanais e mensais automaticamente"""
    try:
        print("🏔️ VERIFICANDO PICOS...")
        hoje = datetime.utcnow().date()
        inicio_semana = hoje - timedelta(days=hoje.weekday())
        inicio_mes = hoje.replace(day=1)
        ...
        for pzem_id in [1, 2]:
            ...
            if potencia_atual <= 10:  # Ignorar valores muito baixos
                continue
            
            print(f"🔍 PZEM{pzem_id}: {potencia_atual:.1f}W")
            
            # ✅ PICOS DIÁRIOS
            pico_diario_existente = DailyPeak.query.filter(
                DailyPeak.date == hoje,
                DailyPeak.pzem_id == pzem_id
            ).first()
            
            if not pico_diario_existente or potencia_atual > pico_diario_existente.value:
                if pico_diario_existente:
                    pico_diario_existente.value = potencia_atual
                    pico_diario_existente.time = datetime.utcnow().time()
                    print(f"🔄 Pico DIÁRIO PZEM{pzem_id} atualizado: {potencia_atual:.1f}W")
                else:
                    novo_pico = DailyPeak(
                        date=hoje,
                        pzem_id=pzem_id,
                    ...
                    db.session.add(novo_pico)
                    print(f"✅ NOVO pico DIÁRIO PZEM{pzem_id}: {potencia_atual:.1f}W")
            
            # ✅ PICOS SEMANAIS
            pico_semanal_existente = WeeklyPeak.query.filter(
                WeeklyPeak.week_start == inicio_semana,
                WeeklyPeak.pzem_id == pzem_id
            ).first()
            
            if not pico_semanal_existente or potencia_atual > pico_semanal_existente.value:
                if pico_semanal_existente:
                    pico_semanal_existente.value = potencia_atual
                    pico_semanal_existente.time = datetime.utcnow().time()
                    print(f"🔄 Pico SEMANAL PZEM{pzem_id} atualizado: {potencia_atual:.1f}W")
                else:
                    novo_pico = WeeklyPeak(
                        week_start=inicio_semana,
                        pzem_id=pzem_id,
                    ...
                    db.session.add(novo_pico)
                    print(f"✅ NOVO pico SEMANAL PZEM{pzem_id}: {potencia_atual:.1f}W")
            
            # ✅ PICOS MENSAIS
            pico_mensal_existente = MonthlyPeak.query.filter(
                MonthlyPeak.month == hoje.month,
                MonthlyPeak.year == hoje.year,
                MonthlyPeak.pzem_id == pzem_id
            ).first()
            
            if not pico_mensal_existente or potencia_atual > pico_mensal_existente.value:
                if pico_mensal_existente:
                    pico_mensal_existente.value = potencia_atual
                    pico_mensal_existente.time = datetime.utcnow().time()
                    print(f"🔄 Pico MENSAL PZEM{pzem_id} atualizado: {potencia_atual:.1f}W")
                else:
                    novo_pico = MonthlyPeak(
                        month=hoje.month,
                        year=hoje.year,
                    ...
                    db.session.add(novo_pico)
                    print(f"✅ NOVO pico MENSAL PZEM{pzem_id}: {potencia_atual:.1f}W")
        
        # ✅ SALVAR TODOS OS PICOS DE UMA VEZ
        db.session.commit()
        print("💾 Todos os picos salvos com sucesso!")
        
    except Exception as e:
        print(f"❌ Erro ao salvar picos: {e}")
        db.session.rollback()

# ==========================================================
# ROTAS PARA VERIFICAR PICOS
# ==========================================================

@app.route('/api/debug/picos', methods=['GET'])
@login_required
def debug_picos():
    """Debug dos picos armazenados"""
    try:
        hoje = datetime.utcnow().date()
        inicio_semana = hoje - timedelta(days=hoje.weekday())
        
        picos_diarios = DailyPeak.query.filter(DailyPeak.date >= hoje - timedelta(days=3)).all()
        picos_semanais = WeeklyPeak.query.filter(WeeklyPeak.week_start >= inicio_semana - timedelta(weeks=2)).all()
        picos_mensais = MonthlyPeak.query.filter(MonthlyPeak.year >= hoje.year).all()
...
@app.route('/api/picos/limpar', methods=['POST'])
@login_required
def limpar_picos():
    """Limpar todos os picos (apenas para desenvolvimento)"""
    try:
        DailyPeak.query.delete()
        WeeklyPeak.query.delete()
        MonthlyPeak.query.delete()
        db.session.commit()
        return jsonify({"success": True, "message": "Todos os picos foram limpos"})
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
